...
CREATE TABLE IF NOT EXISTS `mydb`.`Persona` (
  ...
  PRIMARY KEY (`DNI`),
  INDEX `DNI_idx` (`DNI_cabeza` ASC) VISIBLE,
  CONSTRAINT `DNI`
...
CREATE TABLE IF NOT EXISTS `mydb`.`Piso` (
  ...
  INDEX `DNI_dueño_idx` (`DNI_dueño` ASC) VISIBLE,
  INDEX `Bloque_idx` (`Calle` ASC, `Numero` ASC) VISIBLE,
  CONSTRAINT `Bloque`
...
CREATE TABLE IF NOT EXISTS `mydb`.`Vivienda` (
  ...
  INDEX `Persona_idx` (`DNI_dueño` ASC) VISIBLE,
  CONSTRAINT `Zona`
    FOREIGN KEY (`Nombre_zona`)
    REFERENCES `mydb`.`Zona` (`Nombre`)
    ON DELETE NO ACTION
